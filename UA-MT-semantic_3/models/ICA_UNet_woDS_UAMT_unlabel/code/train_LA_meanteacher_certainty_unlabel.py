...
def validate(iter_num, args, valloader, model, ema_model, snapshot_path):
    ...
    for volume_batch, label_batch, volume_gray, _ in valloader:
        ...
        if np.mod(iter_num, args.sample_freq) == 0:
            ...
            sample_image = torch.cat((sample_image, torch.unsqueeze(label_batch[0, 2, :, :], 0).unsqueeze(1)
                                      , torch.unsqueeze(outputs[0, 2, :, :], 0).unsqueeze(1)), 0)
            sample_image = torch.cat((sample_image, torch.unsqueeze(label_batch[0, 3, :, :], 0).unsqueeze(1)
                                      , torch.unsqueeze(outputs[0, 3, :, :], 0).unsqueeze(1)), 0)
