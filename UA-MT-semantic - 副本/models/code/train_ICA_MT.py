...
parser.add_argument('--supervised_loss', default='EnergyLoss_new',
                    choices=LOSS_NAMES,
                    help='loss: ' +
                         ' | '.join(LOSS_NAMES) +
                         ' (default: BCEDiceLoss)')
...
# added
parser.add_argument('--sample_freq', default=20, type=int, help='sample during validation')
...
def train(iter_num, args, trainloader, model, ema_model, supervised_criterion, consistency_criterion, optimizer, lr_):
    ...
    for volume_batch, label_batch, _, _ in trainloader:
        ...
        if args.supervised_loss == 'EnergyLoss' or args.supervised_loss == 'EnergyLoss_new':
            ...
            score2 = (0.5 - score1)
            supervised_loss = supervised_criterion(score2, label_batch[:args.labeled_bs])
        else:
            supervised_loss = supervised_criterion(torch.unsqueeze(outputs[:args.labeled_bs, 0], 1),
        ...
        iou = iou_score(torch.unsqueeze(outputs[:args.labeled_bs,0],1), label_batch[:args.labeled_bs])
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def validate(iter_num, args, valloader, model, ema_model, snapshot_path):
    ...
    for volume_batch, label_batch, volume_gray, _ in valloader:
        ...
        iou = iou_score(torch.unsqueeze(outputs[:, 0], 1), label_batch)
        rec = recall(torch.unsqueeze(outputs[:, 0], 1), label_batch)
        spec = specificity(torch.unsqueeze(outputs[:, 0], 1), label_batch)
        ...
        betti_num = betti_number(torch.unsqueeze(outputs[:, 0], 1), label_batch)

        val_metric1 = iou + rec
        val_metric2 = 0.25 * over + 0.25 * under + 0.5 * betti_num / 50
        ...
        writer.add_scalar('val/metric', val_metric2, iter_num)

        if iter_num > 400:
            if val_metric1 > args.best_metric1:  # 模型的保存条件 对比iou的最佳值
                save_mode_path = os.path.join(snapshot_path, 'iter_' + str(iter_num) + 'IR' + '.pth')
                torch.save(model.state_dict(), save_mode_path)
                logging.info("save model to {}".format(save_mode_path))
                args.best_metric1 = val_metric1
                print("=> saved best IR model")
            elif val_metric2 < args.best_metric2:  # 模型的保存条件 对比iou的最佳值
                save_mode_path = os.path.join(snapshot_path, 'iter_' + str(iter_num) + 'OUB' + '.pth')
                torch.save(model.state_dict(), save_mode_path)
                logging.info("save model to {}".format(save_mode_path))
                args.best_metric2 = val_metric2
                print("=> saved best OUB model")
            elif betti_num < args.best_betti:  # 模型的保存条件 对比iou的最佳值
                save_mode_path = os.path.join(snapshot_path, 'iter_' + str(iter_num) + 'BT' + '.pth')
                torch.save(model.state_dict(), save_mode_path)
                logging.info("save model to {}".format(save_mode_path))
                args.best_betti = betti_num
                print("=> saved best BT model")

        if np.mod(iter_num, args.sample_freq) == 0:
            sample_image = torch.cat(
                (torch.unsqueeze(volume_gray[0, :, :, :], 0), torch.unsqueeze(label_batch[0, :, :, :], 0)), 0)
            sample_image = torch.cat(
                (sample_image, torch.unsqueeze(outputs[0, 0, :, :], 0).unsqueeze(1)), 0)
            for i in range(1, outputs.size(0)):
                sample_image = torch.cat((sample_image, torch.unsqueeze(volume_gray[i, :, :, :], 0),
                                          torch.unsqueeze(label_batch[i, :, :, :], 0),
                                          torch.unsqueeze(outputs[i, 0, :, :], 0).unsqueeze(1)), 0)
...
if __name__ == "__main__":
    ...
    iter_num = 0
    args.best_metric1 = 0
    args.best_metric2 = 1000
    args.best_betti = 1000
    lr_ = base_lr
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        if args.scheduler == 'CosineAnnealingLR':
            ...
            scheduler.step(args.best_betti)
            lr_ = optimizer.param_groups[0]['lr']
